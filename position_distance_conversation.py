    ...
    for i, item in enumerate(image_info['objects']):
        ...
        if q_type == 'closest':
            ...
            for other_obj in image_info['objects']:
                ...
                dist = calculate_3d_distance(other_obj['3D_location'], item['3D_location'])
                if direction in rels and dist < min_dist:
                    flag = 1
                    min_dist = dist
            ...
            for other_obj in image_info['objects']:
                rels = get_relation(other_obj, item, extrinsic)  # other_obj相较于item的关系
                dist = calculate_3d_distance(other_obj['3D_location'], item['3D_location'])
                if direction in rels and dist > max_dist:
                    flag = 1
                    max_dist = dist
        ...
        target_dist = round(target_dist*100, 2)#单位：厘米
        obj_coor = [round(x, 2) for x in
        ...
        q1 = f"What is the {q_type} object {direction_text} the {desc}?"
        # a1 = f"The {target_obj['category']}, which is {target_dist} centimeters away from {desc}"
        ...
        option_labels = ['(a)', '(b)', '(c)']
        q2 = f"What is the {q_type} object {direction_text} the {desc}?\nThe options are:\n"
        q2 += '\n'.join([f"{label}. {opt}" for label, opt in zip(option_labels, options)])
        a2 = option_labels[options.index(a1)]

        cot = f"""To figure out the {q_type} object {direction_text} {desc}, we should know locations of objects and then compute distance. First, the 3d location of {desc} in camera coordinate system is {obj_coor} meters. Among all objects {direction_text} {desc}, the {target_obj['category']} is the {q_type} from {desc}, and the 3d location of {target_obj['category']} in camera coordinate system is {target_coor} meters. And its distance to {desc} is {target_dist} centimeters."""
        short_output.append({
            "conversation": [
            ...
            "images": [image_path],
            "cot": cot,
            "task_category": "middle_level(position_distance)"
        })
        cot_output.append({
            "conversation": [
                {"human": q1,
